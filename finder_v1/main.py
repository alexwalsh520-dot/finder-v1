...
from .apify_client import AnthropicHTTPClient, ApifyHTTPClient, GroqHTTPClient
from .config import DB_PATH, DOC_ACTOR, ENRICH_ACTOR, LOCK_PATH, MIN_FOLLOWERS, OUTPUT_DIR, SIMILAR_ACTOR, api_config, ensure_dirs
from .db import FinderDB
from .email_search import best_candidate, classify_email, deep_email_search, discover_youtube_urls
...
def parse_args() -> argparse.Namespace:
    ...
    run_parser.add_argument("--target-emails", type=int, default=25)

    daily_parser = subparsers.add_parser("daily-run", help="Keep running until the daily target is hit or the day ends")
    daily_parser.add_argument("--target-emails", type=int, default=DEFAULT_DAILY_TARGET)
    daily_parser.add_argument("--seed-batch-size", type=int, default=DEFAULT_DAILY_BATCH_SIZE)
    daily_parser.add_argument("--max-cycles", type=int, default=25)
...
def unfinished_cached_profiles(db: FinderDB, seed: str) -> List[Dict[str, Any]]:
    rows = list(db.conn.execute(
        ...
        where source_seed = ?
          ...
          and (best_email is null or best_email_type not in ('personal', 'management'))
...
def filter_existing_live_email_profiles(
    ...
    for profile in profiles:
        ...
        if live_row and (
            (live_row.get("email") or "").strip()
            or live_row.get("sent_to_smartlead") is True
            or live_row.get("smartlead_campaign_id")
            or live_row.get("smartlead_sent_at")
        ):
            skipped += 1
            continue
...
def write_csvs(db: FinderDB, run_id: int) -> Dict[str, str]:
    ...
    with final_path.open("w", newline="") as fh:
        ...
        for row in export_rows:
            if row["best_email"] and row["best_email_type"] in ("personal", "management"):
...
def refresh_saved_results(db: FinderDB, supabase: SupabaseClient | None = None) -> Counter:
    ...
    for row in db.iter_email_candidates():
        ...
        keep = email_type in ("personal", "management")
    ...
    for handle in db.list_creator_handles():
        ...
        candidates = [
            ...
            if row["keep"] and row["email_type"] in ("personal", "management")
        ]
...
def import_dashboard_database() -> Dict[str, Any]:
    ...
    for row in delivered:
        ...
        if email_type not in ("personal", "management"):
...
def harvest_doc_job(
    ...
    creator = db.get_creator(handle)
    if creator and creator["best_email"] and creator["best_email_type"] in ("personal", "management"):
        db.mark_doc_job_completed(job["apify_run_id"], "Creator already has a kept email.")
        return False
    ...
    for candidate in candidates:
        if db.has_email_candidate(handle, candidate["email"], candidate["source_method"]):
            continue
        keep = candidate["email_type"] in ("personal", "management")
    ...
    best = best_candidate(new_candidates)
    if best:
        profile = db.get_profile(handle)
...
def check_doc_jobs(
    ...
    if jobs:
        print(f"Checking {len(jobs)} pending DOC job(s)...")
    for job in jobs:
        try:
            if harvest_doc_job(db, counts, apify, supabase, dict(job)):
                harvested += 1
        except Exception as exc:
...
def load_fallback_seed_bank(db: FinderDB) -> List[str]:
    seeds: List[str] = []
    for seed in DEFAULT_FALLBACK_SEEDS:
    ...
    for row in rows:
        ...
        if handle and handle not in seeds:
            seeds.append(handle)
    for row in db.list_seed_performance():
        handle = (row["seed_handle"] or "").strip().lower()
        if handle and handle not in seeds:
            seeds.append(handle)
    return seeds


def build_seed_rankings(db: FinderDB, day: str) -> List[Dict[str, Any]]:
    ...
    for fallback_index, seed in enumerate(load_fallback_seed_bank(db)):
        ...
        exploration_bonus = 3.0 if not perf else 0.0
        fallback_bonus = max(0.0, 2.0 - (fallback_index * 0.1))
        recent_penalty = 0.0
        if usage.get("last_outcome") == "no_progress":
            recent_penalty = 100.0
        elif usage:
            recent_penalty = 5.0
        score = (
            (total_kept * 6.0)
            + (total_doc_harvested * 4.0)
            + (qualified_rate * 25.0)
            + (kept_rate * 55.0)
            + (doc_rate * 10.0)
            + exploration_bonus
...
def select_seed_batch(
    ...
    rankings = build_seed_rankings(db, day)
    unused = [row["seed_handle"] for row in rankings if not row["used_today"]]
    if unused:
        return unused[:batch_size]
    if zero_progress_cycles >= ZERO_PROGRESS_ROTATION_THRESHOLD:
        retryable = [row["seed_handle"] for row in rankings if row["last_outcome_today"] != "no_progress"]
    else:
        retryable = [row["seed_handle"] for row in rankings]
    return retryable[:batch_size]


def write_daily_seed_report_csv(db: FinderDB, day: str) -> str:
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    path = OUTPUT_DIR / f"daily_{day}_seed_report.csv"
    rows = build_seed_rankings(db, day)
...
def write_daily_summary_json(
    ...
    summary = {
        ...
        "remaining_to_goal": max(target_emails - int(counts.get("current_daily_count") or counts.get("existing_today") or 0), 0),
        ...
        "top_seed_candidates": build_seed_rankings(db, day)[:10],
...
def upload_output_files(
    ...
    *,
    ...
    if not supabase.ensure_storage_bucket(bucket_name, public=False):
        return uploaded
    wanted = ["review", "summary", "seed_report"]
...
def pace_status(current_count: int, target_emails: int, started_at: str, zero_progress_cycles: int, last_progress_at: str | None) -> str:
    if current_count >= target_emails:
        return "complete"
    if zero_progress_cycles >= 3:
        return "stalled"
    progress_time = None
    if last_progress_at:
        try:
...
def run_seed_cycle(
    ...
    if supabase.enabled():
        ...
        cycle["skipped_existing_live_email"] += skipped_existing_live
    ...
    harvested_before = check_doc_jobs(db, counts, apify, supabase, limit=DEFAULT_DOC_CHECK_LIMIT)
    if harvested_before:
        cycle["doc_emails_harvested"] += harvested_before
        print(f"  Harvested {harvested_before} DOC email(s) before processing @{seed}")

    for profile in queue:
        handle = profile["username"]
        try:
            process_profile(db, counts, profile, apify, anthropic, youtube_key, supabase, seed)
    ...
    harvested_after = check_doc_jobs(db, counts, apify, supabase, limit=DEFAULT_DOC_CHECK_LIMIT)
    if harvested_after:
...
def process_profile(
    ...
    if existing and existing["best_email"] and existing["best_email_type"] in ("personal", "management"):
        return False
    if existing and existing["qualified"] == 0 and existing["reject_reason"] != "under_100k":
        return False
    ...
    initial = deterministic_qualification(profile)
    result = initial
    if initial.get("needs_ai") and anthropic.enabled():
        try:
            result = ai_qualification(profile, anthropic)
        except Exception as exc:
            result = initial
            result["why"] = f"{initial['why']} AI fallback failed: {exc}"
    ...
    db.set_qualification(
        ...
        result.get("why") or "",
        "ai" if initial.get("needs_ai") and anthropic.enabled() else QUALIFICATION_VERSION,
    )
    ...
    best = best_candidate(email_candidates)

    if not best and apify.enabled():
        ...
        best = best_candidate(youtube_candidates)
...
def run_pipeline(seeds: List[str], target_emails: int) -> None:
    ...
    counts = Counter()
    paths = {"final": "", "audit": "", "review": ""}
    if supabase.enabled():
        todays_existing = supabase.count_today_emails(today_business_date())
    ...
    try:
        if apify.enabled():
            harvested = check_doc_jobs(db, counts, apify, supabase, limit=100)
            if harvested:
        ...
        if supabase.enabled() and counts["existing_today"] < target_emails:
            ...
            for profile in inventory_profiles:
                ...
                try:
                    process_profile(db, counts, profile, apify, anthropic, youtube_key, supabase, "supabase_inventory")
                    checkpoint_outputs_with_review(db, run_id, counts, started_at)
                except Exception as exc:
                    counts["profile_errors"] += 1
        ...
        if apify.enabled():
            check_doc_jobs(db, counts, apify, supabase, limit=100)
...
def run_daily(target_emails: int, seed_batch_size: int, max_cycles: int, hard_stop_hour_local: int) -> None:
    ...
    run_id = db.create_run(seeds, target_emails)
    counts = Counter()
    counts["existing_today"] = supabase.count_today_emails(day) if supabase.enabled() else 0
    ...
    daily_paths = {"final": "", "audit": "", "review": "", "summary": "", "seed_report": ""}
    fatal_error: Exception | None = None
    try:
        if apify.enabled() and current_count < target_emails:
            harvested = check_doc_jobs(db, counts, apify, supabase, limit=DEFAULT_DOC_CHECK_LIMIT)
            if harvested:
                current_count = (supabase.count_today_emails(day) if supabase.enabled() else db.count_kept_emails_since(started_at))
        ...
        while cycles_completed < max_cycles:
            ...
            seed_batch = select_seed_batch(db, day, seed_batch_size, zero_progress_cycles)
            if not seed_batch:
                print("No fresh seeds available for today.")
                break
            ...
            counts["pace_status"] = pace_status(current_count, target_emails, started_at, zero_progress_cycles, last_progress_at)
        ...
        counts["cycles_completed"] = cycles_completed
        counts["zero_progress_cycles"] = zero_progress_cycles
        counts["pace_status"] = "error" if fatal_error else pace_status(current_count, target_emails, started_at, zero_progress_cycles, last_progress_at)
        status = "error" if fatal_error else ("complete" if current_count >= target_emails else ("stalled" if counts["pace_status"] == "stalled" else "partial"))
        try:
            paths = checkpoint_outputs_with_review(db, run_id, counts, started_at)
    ...
    try:
        if args.command == "status":
            ...
            with pipeline_lock(LOCK_PATH, "check-doc", {"limit": args.limit}):
                ...
                if supabase.enabled():
                    counts["existing_today"] = supabase.count_today_emails(today_business_date())
                    print(f"Existing centralized emails for today: {counts['existing_today']}")
                harvested = 0
                doc_status = "success"
                try:
                    harvested = check_doc_jobs(db, counts, apify, supabase, limit=args.limit)
                except Exception:
                    doc_status = "error"
                    raise
                finally:
